...
class API {
  static dataLoader(
    ...
    return function*() {
      ...
      const authorizedToken = localStorage.getItem('shareBigyaSecret')
        ? `Bearer ${localStorage.getItem('shareBigyaSecret')}`
        : '';
      try {
        let options;
        if (data !== undefined) {
  ...
  static delete(apiUri, onSuccess, onError, ...actionArguments) {
    return function*() {
      ...
      try {
        console.log(apiUri, onSuccess, onError);
        const options = {
          method: 'DELETE',
          headers: {
            // 'Content-Type': 'application/json',
            ...
            'X-Requested-With': 'XMLHttpRequest',
            // 'Access-Control-Allow-Origin': '*',
            Authorization: localStorage.getItem('shareBigyaSecret')
              ? `Bearer ${localStorage.getItem('shareBigyaSecret')}`
              : '',
          },
        };
  ...
  static async getData(apiUri, token = '') {
    try {
      ...
      if (status === 401) {
        localStorage.removeItem('shareBigyaSecret');
        history.push('/');
